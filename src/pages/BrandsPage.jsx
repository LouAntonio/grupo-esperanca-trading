					...
					<div className="services-grid">
						{services.map((s, i) => (
							<div key={s.id}
								...
								style={{
									background: '#fff', borderRadius: 'var(--radius-md)', padding: '32px 24px',
									boxShadow: 'var(--shadow-card)', transition: 'transform 0.3s, box-shadow 0.3s',
							...
							>
								<div className="service-card-icon"
									style={{
										...
										marginBottom: '16px', fontSize: '20px', color: 'var(--color-green)',
									}}
								>
									<i className={`fas ${i % 2 === 0 ? 'fa-leaf' : 'fa-apple-alt'}`} />
								</div>
								<h3 style={{ fontFamily: "'Rubik', sans-serif", fontSize: '17px', fontWeight: 600, color: 'var(--color-dark)', margin: '0 0 8px' }}>
									{t(s.titleKey)}
								</h3>
								<p style={{ fontSize: '14px', lineHeight: '1.7', color: 'var(--color-muted)', margin: 0 }}>
									{t(s.descriptionKey)}
								</p>
							</div>
						))}
